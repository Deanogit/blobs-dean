...
function setup() {
  ...
  while (totalArea < maxArea && particles.length < maxVertexCount) {
    ...
    const radius = Math.min(
      radiusLimit,
      (random(1) ** 3 * (maxRadius - minRadius) + minRadius) * minLength
    );
    offsetY += prevRadius + radius + 50;
    const blob = generateBlob(
      width / 2 + random(-1, 1) * (width / 2 - marginX - radius),
      height / 2 - offsetY,
      radius
    );
    totalArea += blob.area;
...
function draw() {
  ...
  for (let substep = substeps; substep--; ) {
    ...
    for (let i = particles.length; i--; ) {
      ...
      particle.constrain(marginX, -99999, width - marginX, height - marginY);
